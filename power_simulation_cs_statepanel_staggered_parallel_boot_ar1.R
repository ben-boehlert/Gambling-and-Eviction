...
for (k in seq_len(nrow(effect_tbl))) {
  ...
  while (n_done < N_SIMS) {
    ...
    batch_out <- if (use_parallel) {
      parallel::parLapply(cl, seeds_batch, function(ss) one_sim(ss, eff_log))
    } else {
...
if (any(abs(pwr$effect_pct) < 1e-12)) {
  ...
  diag_out <- if (use_parallel) {
    parallel::parLapply(cl, sim_seeds[seq_len(n_diag)], function(ss) one_sim(ss, 0.0))
  } else {
    lapply(sim_seeds[seq_len(n_diag)], function(ss) one_sim(ss, 0.0))
  }
