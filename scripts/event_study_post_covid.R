#!/usr/bin/env Rscript
################################################################################
# event_study_post_covid.R
#
# Event study for states treated AFTER September 2021 (post-COVID period)
# Excludes pre-COVID adopters from the analysis
################################################################################
...
cat("\n")
cat("================================================================================\n")
cat("EVENT STUDY: POST-COVID PERIOD (Treatment Sept 2021 or later)\n")
cat("================================================================================\n\n")
...
cat(sprintf("  Post-COVID treated states: %d\n",
           sum(panel_data_postcovid$treat_period == "Post-COVID" & !duplicated(panel_data_postcovid$state_abb))))
cat(sprintf("  Never-treated states: %d\n\n",
           sum(panel_data_postcovid$treat_period == "Never" & !duplicated(panel_data_postcovid$state_abb))))

# Create event time bins
cat("Creating event time bins...\n")
panel_data_postcovid <- panel_data_postcovid %>%
  mutate(
    event_bin = case_when(
...
panel_reg <- panel_data_postcovid %>%
  filter(event_bin != "-1") %>%
  mutate(
...
p <- ggplot(results, aes(x = event_time, y = coef)) +
  ...
  scale_shape_manual(values = c("TRUE" = 1, "FALSE" = 19),
                     ...
                     name = "") +
  labs(
    title = "Event Study: Post-COVID Adopters (Treatment Sept 2021 or later)",
...
ggsave("output/csdid_pretrends/post_covid/event_study_post_covid.pdf", p, width = 12, height = 7)
ggsave("output/csdid_pretrends/post_covid/event_study_post_covid.png", p, width = 12, height = 7, dpi = 300)
write.csv(results, "output/csdid_pretrends/post_covid/event_study_results.csv", row.names = FALSE)
...
cat("Output files:\n")
cat("  • output/csdid_pretrends/post_covid/event_study_post_covid.pdf\n")
cat("  • output/csdid_pretrends/post_covid/event_study_results.csv\n\n")
