...
cfg <- list(
  ...
  data_dir = "data/raw",   # e.g. "." locally, or "/scratch/..." on Della
  ...
  tryCatch({
    ...
    if (cfg$estimand == "overall_att") {
      ...
      agg_result <- tryCatch({
        ...
        # For clustered SEs with bootstrap, we need to use bootstrap aggregation
        # The analytical influence function approach doesn't work for clustered SEs
        if (!is.null(att$se) && length(att$se) == length(att$att)) {
          # Compute SE of the mean assuming independence (conservative):
          # SE(mean) = (1/n) * sqrt(sum(SE_i^2))
          n_att <- sum(!is.na(att$att))
          se_manual <- (1/n_att) * sqrt(sum(att$se^2, na.rm = TRUE))
        } else {
          se_manual <- NA_real_
        }
