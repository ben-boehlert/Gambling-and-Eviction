#!/usr/bin/env Rscript
################################################################################
# power_simulation_cs_v4_6_logcounts_heatmap80.R
#
...
set.seed(20251224)
# Progress output can get noisy/fragile under parallel futures on HPC.
# Enable progress only when explicitly requested.
handlers(global = interactive())
if (interactive()) handlers("txtprogressbar")
...
slurm_cpus_hint <- function() {
  # Robustly infer CPU allocation under Slurm, even if SLURM_CPUS_PER_TASK is missing.
  # Returns list(workers=..., note=...)
  cpus_per_task <- suppressWarnings(as.integer(Sys.getenv("SLURM_CPUS_PER_TASK", NA)))
  ntasks        <- suppressWarnings(as.integer(Sys.getenv("SLURM_NTASKS", NA)))
  cpus_on_node  <- suppressWarnings(as.integer(Sys.getenv("SLURM_CPUS_ON_NODE", NA)))
  job_cpus_node <- Sys.getenv("SLURM_JOB_CPUS_PER_NODE", NA_character_)

  # Parse "20(x1)" or "20" or "20,20"
  job_cpus_parsed <- NA_integer_
  if (!is.na(job_cpus_node) && nzchar(job_cpus_node)) {
    # take the first integer we see
    mm <- regmatches(job_cpus_node, regexpr("\\d+", job_cpus_node))
    if (length(mm) == 1 && nzchar(mm)) job_cpus_parsed <- suppressWarnings(as.integer(mm))
  }

  # If Slurm is present, prefer cpus-per-task (within one R process).
  if (is.finite(cpus_per_task) && cpus_per_task >= 1L) {
    note <- if (is.finite(ntasks) && ntasks > 1L) {
      "SLURM_NTASKS>1 detected; using SLURM_CPUS_PER_TASK (per-task workers)"
    } else {
      "Using SLURM_CPUS_PER_TASK"
    }
    return(list(workers = cpus_per_task, note = note))
  }

  # Next best hints
  if (is.finite(job_cpus_parsed) && job_cpus_parsed >= 1L) return(list(workers = job_cpus_parsed, note = "Using SLURM_JOB_CPUS_PER_NODE"))
  if (is.finite(cpus_on_node)   && cpus_on_node   >= 1L) return(list(workers = cpus_on_node,   note = "Using SLURM_CPUS_ON_NODE"))
  if (is.finite(ntasks)         && ntasks         >= 1L) return(list(workers = ntasks,         note = "Using SLURM_NTASKS"))

  # Not on Slurm (or env vars missing)
  list(workers = NA_integer_, note = "No Slurm CPU env vars detected")
}

print_parallel_diagnostics <- function() {
  # Helpful when you *think* you asked for 20 but the job only sees 4.
  envs <- c("SLURM_JOB_ID","SLURM_JOB_NAME","SLURM_CPUS_PER_TASK","SLURM_NTASKS",
            "SLURM_CPUS_ON_NODE","SLURM_JOB_CPUS_PER_NODE","SLURM_MEM_PER_NODE","SLURM_MEM_PER_CPU")
  vals <- Sys.getenv(envs, unset = "")
  diag_tbl <- tibble::tibble(var = envs, value = vals)
  message("Slurm env diagnostics:")
  suppressMessages(print(diag_tbl))

  if (isTRUE(has_parallelly)) {
    ac <- tryCatch(parallelly::availableCores(), error = function(e) NA_integer_)
    message(glue("parallelly::availableCores() = {ac}"))
  }
}
...
if (!exists("cfg", inherits = FALSE)) {
  cfg <- list(
    ...
    outcome_preference = c(
      # Default: log(counts), not rates
      "log_filings_count",
      "log_filings_2020",
      "log_filings_avg",
      "log_filings_avg_prepandemic_baseline",
      # Fallbacks (raw counts)
      "filings_count",
      "filings_2020",
      "filings_avg",
    ...
    # "auto" (default) uses Slurm allocation if present; otherwise uses available cores.
    workers_mode = "auto",

    # Chunk draws so you get regular "heartbeat" logs even under parallel futures
    draw_chunk_size = 50,
...
safe_future_map_draws_ids <- function(sim_ids, one_draw_fn, eff, cfg) {
  # Try parallel with decreasing workers; if still failing, fall back to sequential.
  # Never throws; returns a tibble with length(sim_ids) rows (possibly all-NA with fail tag).
  sim_ids <- as.integer(sim_ids)

  if (!isTRUE(cfg$use_parallel)) {
    return(purrr::map_dfr(sim_ids, ~ one_draw_fn(eff, .x)))
  }
  ...
  out2 <- tryCatch({
    purrr::map_dfr(sim_ids, ~ one_draw_fn(eff, .x))
  }, error = function(e) {
    tibble::tibble(
      sim = sim_ids,
...
safe_future_map_draws_chunked <- function(n_sims, one_draw_fn, eff, cfg, chunk_size = NULL) {
  # Chunk draws so you get regular "heartbeat" logs even under parallel futures.
  n_sims <- as.integer(n_sims)
  if (n_sims <= 0) return(tibble(sim = integer(0), p = numeric(0), est = numeric(0), se = numeric(0), fail = character(0)))

  chunk_size <- as.integer(chunk_size %||% (cfg$draw_chunk_size %||% 50L))
  chunk_size <- max(1L, chunk_size)

  sim_ids <- seq_len(n_sims)
  chunks <- split(sim_ids, ceiling(seq_along(sim_ids) / chunk_size))

  purrr::imap_dfr(chunks, function(ids, k) {
    message(glue("[{Sys.time()}] eff={eff}: draws {min(ids)}-{max(ids)} / {n_sims}"))
    flush.console()
    safe_future_map_draws_ids(ids, one_draw_fn, eff, cfg)
  })
}

safe_future_map_draws <- function(n_sims, one_draw_fn, eff, cfg) {
  safe_future_map_draws_chunked(
    n_sims = n_sims,
    one_draw_fn = one_draw_fn,
    eff = eff,
    cfg = cfg,
    chunk_size = cfg$draw_chunk_size %||% 50L
  )
}
...
prep_panel_counties <- function(cfg) {
  ...
  df <- readr::read_csv(path, show_col_types = FALSE) %>%
    ...
    mutate(
      ...
      log_filings_count = if_else(!is.na(filings_count) & as.numeric(filings_count) >= 0, log1p(as.numeric(filings_count)), NA_real_)
...
prep_panel_states_from_counties <- function(cfg) {
  ...
  cfg2 <- cfg
  cfg2$outcome_preference <- c("log_filings_count", cfg$outcome_preference)
...
prep_panel_states_ets <- function(cfg) {
  ...
  # Use log(counts) outcomes by default
  if ("filings_2020" %in% names(df)) df <- df %>% mutate(log_filings_2020 = log1p(pmax(as.numeric(filings_2020), 0)))
  if ("filings_avg"  %in% names(df)) df <- df %>% mutate(log_filings_avg  = log1p(pmax(as.numeric(filings_avg),  0)))
...
prep_panel_sites_ets <- function(cfg) {
  ...
  if ("filings_2020" %in% names(df)) df <- df %>% mutate(log_filings_2020 = log1p(pmax(as.numeric(filings_2020), 0)))
  if ("filings_avg"  %in% names(df)) df <- df %>% mutate(log_filings_avg  = log1p(pmax(as.numeric(filings_avg),  0)))
  if ("filings_avg_prepandemic_baseline" %in% names(df)) df <- df %>% mutate(log_filings_avg_prepandemic_baseline = log1p(pmax(as.numeric(filings_avg_prepandemic_baseline), 0)))
...
simulate_power_grid_states_switchers <- function(panel_df,
  ...
  if (!is.null(n_states_grid)) {
    max_clusters <- if (cluster_var %in% names(baseline_full)) dplyr::n_distinct(baseline_full[[cluster_var]], na.rm = TRUE) else NA_integer_
  ...
  out_all <- purrr::pmap_dfr(grid, function(n_states, n_switchers) {
    ...
    res <- tryCatch({
      ...
      n_units_s <- if (!is.null(baseline_s)) dplyr::n_distinct(baseline_s$unit_id) else NA_integer_
      n_clusters_s <- if (!is.null(baseline_s) && cluster_var %in% names(baseline_s)) {
        dplyr::n_distinct(baseline_s[[cluster_var]], na.rm = TRUE)
      } else {
        NA_integer_
      }
      start_s <- if (!is.null(baseline_s)) min(baseline_s$month_date, na.rm = TRUE) else as.Date(NA)
      end_s   <- if (!is.null(baseline_s)) max(baseline_s$month_date, na.rm = TRUE) else as.Date(NA)
    ...
    try({
      existing <- if (file.exists(chk)) {
        suppressMessages(readr::read_csv(chk, show_col_types = FALSE))
      } else {
        tibble()
      }
      suppressWarnings(readr::write_csv(bind_rows(existing, res), chk))
      rm(existing)
    }, silent = TRUE)
...
run_power_simulation <- function(cfg) {
  ...
  # Decide workers at runtime (important on Slurm, and avoids stale cfg objects)
  if (isTRUE(cfg$use_parallel)) {
    mode <- cfg$workers_mode %||% "auto"
    if (is.null(cfg$workers) || identical(mode, "auto")) {
      hint <- slurm_cpus_hint()
      if (is.finite(hint$workers) && hint$workers >= 1L) {
        cfg$workers <- as.integer(hint$workers)
        message(glue("Workers (auto): {cfg$workers} ({hint$note})"))
      } else {
        cfg$workers <- compute_default_workers(fallback = 1L)
        message(glue("Workers (auto): {cfg$workers} (non-Slurm default)"))
      }
    } else {
      cfg$workers <- as.integer(cfg$workers)
      message(glue("Workers (manual): {cfg$workers}"))
    }

    # Print env hints once; helps explain "why 4 not 20"
    print_parallel_diagnostics()
  }
  ...
  if (isTRUE(cfg$run_state_switcher_grid)) {
    ...
    # NEW: Heatmap showing whether each cell achieves the target power (default 80%) at each effect size
    power80_plot <- power_grid_all %>%
      mutate(
        hit_target = power >= (cfg$power_target %||% 0.80),
        n_states = as.factor(n_states),
        n_switchers = as.factor(n_switchers),
        effect_size = factor(effect_size, levels = sort(unique(effect_size)))
      ) %>%
      ggplot(aes(x = n_states, y = n_switchers, fill = hit_target)) +
      geom_tile() +
      facet_grid(option ~ effect_size) +
      labs(
        title = paste0("Cells reaching ", round(100 * (cfg$power_target %||% 0.80)), "% power by effect size"),
        subtitle = paste0("Outcome: ", cfg$outcome_name, " | alpha=", cfg$alpha, " | sims=", cfg$n_sims),
        x = "Number of states (clusters) in sample",
        y = "Number of treated states (switchers)",
        fill = "Meets target?"
      ) +
    ...
    ggsave(file.path(cfg$data_dir, "power_target_heatmap.png"), power80_plot, width = 12, height = 6.5, dpi = 300)
    ...
    message(glue("  - {file.path(cfg$data_dir, 'power_target_heatmap.png')}"))
  } else {
