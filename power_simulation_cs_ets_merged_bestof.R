...
#
# INPUT (required): merged_ets_combined.csv
#
# Purpose:
#   Simulation-based power for staggered adoption DiD using Callaway & Sant'Anna
#   (did::att_gt + did::aggte(type="simple")), with an untreated baseline
#   (never-treated + pre-periods of treated) in the spirit of Black et al.'s
#   simulation approach.
#
# Why this file exists:
#   - You want a single, robust script (no iterative patching) that:
#       * uses log outcomes (log1p),
#       * uses enough sims by default (N_SIMS=5000),
#       * uses untreated baseline variation (never-treated + pre-treated),
#       * uses the full panel of merged_ets_combined.csv,
#       * never emits NA p-values (failures -> p_value=1),
#       * avoids common segfault triggers (threads, parallel flags, DR method).
#
# Hard-crash reality check:
#   If R is *segfaulting*, that's almost always a compiled package problem
#   (did / DRDID / Matrix / OpenMP). But one real fix that often helps is to:
#     - force single-thread BLAS/OpenMP, and
#     - avoid est_method="dr" (DRDID) and use est_method="ipw".
#
# Modes:
#   - DRY_RUN=TRUE: does everything except call did; writes debug_one_draw.csv then exits.
#   - TEST_MODE=TRUE: allows small N_SIMS for quick checks.
#
# Key env vars:
...
#   DRY_RUN=TRUE
#   N_SIMS=5000
#   EFFECT_PCTS=0,0.01,0.02,0.05
#   OUTCOME=log1p_filings_count   # or log1p_filings_avg, filings_count, filings_avg
#   CONTROL_GROUP=notyettreated   # or never_treated
#   DID_EST_METHOD=ipw            # ipw (default), reg, dr (not recommended if you segfault)
#   DID_BSTRAP=FALSE              # bootstrap off by default
#   DID_BITERS=0
################################################################################
...
# ----------------------------- crash safety -----------------------------------
# Thread oversubscription / OpenMP issues can cause hard crashes.
Sys.setenv(
  OMP_NUM_THREADS = "1",
...
ym_to_int <- function(d) { d <- as.Date(d); year(d) * 12L + month(d) }
...
# allow small sims for quick checks in TEST_MODE
if (!cfg$test_mode && (is.na(cfg$n_sims) || cfg$n_sims < 500)) stop_with("N_SIMS must be >= 500 unless TEST_MODE=TRUE (got {cfg$n_sims}).")
...
load_panel <- function(cfg) {
  ...
  # ensure unique unit-time (should already be unique for state-month)
  ...
  # Convert unit id to integer (this matches the stable scripts you used before)
  ...
  run_att <- function(control_group, cluster, bstrap_flag) {
    ...
    withCallingHandlers(
      ...
      warning = function(w) {
        ...
        # keep warnings from spamming output but don't stop the run
...
if (isTRUE(cfg$dry_run)) {
  set.seed(cfg$seed)
  sched <- draw_placebo_schedule(unit_ranges, g_pool0, cfg, n_treat_desired)
...
subset_grid <- if (length(grid_n_states) == 0) data.frame(n_states = n_distinct(baseline$unit_id)) else {
  data.frame(n_states = grid_n_states) %>%
    filter(n_states >= 6, n_states <= n_distinct(baseline$unit_id))
}

one_sim <- function(sim_id, effect_log, baseline_s, unit_ranges_s, g_pool, cfg, n_treat) {
  set.seed(cfg$seed + sim_id)

  sched <- NULL
  for (k in seq_len(cfg$max_redraw)) {
    sched <- tryCatch(draw_placebo_schedule(unit_ranges_s, g_pool, cfg, n_treat), error = function(e) NULL)
  ...
  # NOTE: if did is what segfaults, it will happen here.
