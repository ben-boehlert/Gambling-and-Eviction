...
if (!exists("cfg", inherits = FALSE)) {
  cfg <- list(
    ...
    alpha = 0.05,
    power_target = 0.80,
    mde_method = 'bracket',  # 'bracket' (recommended) or 'closest2'
...
att_gt_safe <- function(df_in, cfg, cluster_var) {
  ...
  # did only honors clustervars when bootstrapping; otherwise it warns and ignores clustering.
...
# did can emit repeated warnings about (i) small groups and (ii) a singular pre-test
# covariance matrix. Those warnings are about ancillary diagnostics (and group sizes), not failures.
# We muffle them to keep simulation logs readable; the underlying estimates/p-values are unchanged.
withCallingHandlers(
...
compute_mde <- function(power_df, power_target = 0.80, method = c('bracket', 'closest2'), clamp = TRUE) {
  ...
  # If we never reach the target within the tested grid, don't extrapolate.
  if (max(df$power, na.rm = TRUE) < power_target) return(NA_real_)

  # If we already exceed target at the smallest tested effect, that's your (conservative) MDE.
  ...
  if (method == 'bracket') {
    ...
    # Stata example style: pick the 2 points closest to the target and fit a local line.
    df2 <- df %>%
...
simulate_power_grid_states_switchers <- function(panel_df,
  ...
  # Cap the requested grid to what's feasible in this baseline (otherwise subsetting will error)
  if (!is.null(n_states_grid)) {
    ...
    if (is.finite(max_clusters)) {
      ...
      if (length(n_states_grid) == 0L) {
        stop(glue::glue("All requested n_states_grid exceed available clusters in baseline: {max_clusters}."))
      }
    }
  }
...
run_power_simulation <- function(cfg) {
  ...
  # Convenience label for plots/text (used in heatmaps and grant paragraph)
  cfg$outcome_name <- attr(panel_df, "outcome_var")
  if (is.null(cfg$outcome_name) || is.na(cfg$outcome_name)) cfg$outcome_name <- "outcome"
  ...
  if (isTRUE(cfg$run_state_switcher_grid)) {
    ...
    mde_surface <- power_grid_all %>%
      ...
      summarise(mde_80 = compute_mde(dplyr::pick(effect_size, power), power_target = cfg$power_target, method = cfg$mde_method), .groups = "drop")
    
    write_csv(mde_surface, file.path(cfg$data_dir, "mde_surface_states_switchers.csv"))
    
# Heatmaps (requested): MDE surface and power at max tested effect
# Note: fill scale left as ggplot default (no custom colors).
...
Saved outputs:")
    ...
    mde_tbl <- power_results %>%
      ...
      summarise(mde_80 = compute_mde(pick(everything()), power_target = cfg$power_target, method = cfg$mde_method), .groups = "drop")
    print(mde_tbl)
    ...
    mde_for_plot <- mde_tbl %>%
      dplyr::filter(!is.na(mde_80)) %>%
      dplyr::arrange(mde_80) %>%
      dplyr::slice(1)
    ...
    if (nrow(mde_for_plot) == 1) {
      p <- p + geom_vline(xintercept = mde_for_plot$mde_80, linetype = "dotted")
    }

    ggsave(file.path(cfg$data_dir, "power_curve.png"), p, width = 8, height = 5, dpi = 300)
    
    
    # Choose a preferred spec for the paragraph.
# If MDE isn't reached within the tested grid, say so explicitly (no 'NA' in prose).
best_mde <- mde_tbl %>%
  filter(!is.na(mde_80)) %>%
  arrange(mde_80) %>%
  slice(1)
...
if (nrow(best_mde) == 0) {
  ...
  mde_sentence <- glue(
    'In preliminary simulations up to an imposed effect of {max_pow_tbl$max_effect} outcome units, estimated power peaks at {round(max_pow_tbl$max_power, 3)}; we will expand the effect-size grid to compute the 80% minimum detectable effect.'
  )
} else {
  mde_sentence <- glue('Under our preferred specification, the minimum detectable effect for 80% power is approximately {round(best_mde$mde_80, 3)} outcome units.')
}
