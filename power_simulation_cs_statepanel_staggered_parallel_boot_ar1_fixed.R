#!/usr/bin/env Rscript
################################################################################
# power_simulation_cs_statepanel_staggered_parallel_boot_ar1.R
#
# Adds TWO options to your existing design:
#   (1) ERR_MODE = iid_month (old) OR ar1 (new; within-state AR(1) errors)
#   (2) DID_BSTRAP / DID_BITERS to use did's bootstrap + p-values when available
################################################################################
...
pval_from_z <- function(z) if (!is.finite(z)) NA_real_ else 2 * pnorm(-abs(z))
...
ERR_AR1_RHO  <- parse_num(getenv1("ERR_AR1_RHO", ""), NA_real_)  # optional override
ERR_AR1_CLIP <- parse_num(getenv1("ERR_AR1_CLIP", "0.98"), 0.98)
if (!is.finite(ERR_AR1_CLIP) || ERR_AR1_CLIP <= 0 || ERR_AR1_CLIP >= 0.999) ERR_AR1_CLIP <- 0.98
...
log_line(RUN_LOG, glue("PWD        : {getwd()}"))
...
log_line(RUN_LOG, glue("ERR_MODE   : {ERR_MODE} (ERR_AR1_RHO={ifelse(is.finite(ERR_AR1_RHO), ERR_AR1_RHO, NA)}, clip={ERR_AR1_CLIP})"))
...
build_state_panel <- function(df) {
  ...
  bind_rows(county_state, state_fallback) %>% arrange(state_abb, month_date)
}
...
if (length(state_list) < 30) {
  stop(glue("Panel build produced only {length(state_list)} states. See states_in_panel_raw.csv"), call. = FALSE)
}
...
g_bad <- panel %>%
  ...
  filter(ng != 1)
if (nrow(g_bad) > 0) stop("g is not constant within state. Fix treat_start.", call. = FALSE)
...
if (DRY_RUN) {
  log_line(RUN_LOG, "DRY_RUN requested: stopping after validation.")
...
if (OUTCOME == "log1p_filings_count") {
  ...
  if (!("filings_per_1k_renters" %in% names(panel))) stop("OUTCOME=log1p_rate but filings_per_1k_renters missing.", call. = FALSE)
...
if (length(missing_t) > 0) {
  log_line(RUN_LOG, glue("WARNING: {length(missing_t)} months have zero untreated obs; dropping those months."))
...
if (any(!is.finite(panel$yhat))) {
  log_line(RUN_LOG, "WARNING: non-finite yhat; dropping those rows.")
...
# Pools for iid_month mode
resid_pool_by_t <- split(base_dat$ehat, base_dat$t)
global_pool <- base_dat$ehat
global_pool <- global_pool[is.finite(global_pool)]
if (length(global_pool) < 20) stop("Too few untreated residuals to simulate.", call. = FALSE)

# Objects for ar1 mode (filled only if ERR_MODE == "ar1")
rho_used <- NA_real_
sigma_u <- NA_real_
sd_row <- NULL
idx_by_id <- NULL
...
if (ERR_MODE == "ar1") {
  log_line(RUN_LOG, "Calibrating AR(1) from untreated residuals...")
  ...
  std_pool_by_t <- split(base_dat$z, base_dat$t)
  std_global_pool <- base_dat$z
...
one_sim <- function(seed, effect_log) {
  ...
  out <- tryCatch({
    suppressWarnings({
      ...
      p <- if (!is.null(agg$overall.pval) && is.finite(as.numeric(agg$overall.pval))) {
...
if (use_parallel) {
  log_line(RUN_LOG, glue("Parallel: using_workers={N_WORKERS}"))
  ...
  parallel::clusterExport(
    cl,
    varlist = c(
      "panel","resid_pool_by_t","global_pool",
      ...
      "draw_errors_iid_month","draw_errors_ar1","pval_from_z","one_sim"
    ),
    envir = environment()
  )
}
...
for (k in seq_len(nrow(effect_tbl))) {
  ...
  while (n_done < N_SIMS) {
    ...
    batch_out <- if (use_parallel) {
      parallel::parLapply(cl, seeds_batch, one_sim, effect_log = eff_log)
    } else {
      lapply(seeds_batch, function(ss) one_sim(ss, effect_log = eff_log))
    }
    t1 <- Sys.time()
    ...
    if (abs(eff_pct) < 1e-12 && ok_c_n > 0) type1_so_far <- mean(p_c[ok_c] < ALPHA, na.rm = TRUE)
    ...
    append_csv(tibble::tibble(
      ...
      elapsed_sec_batch = as.numeric(difftime(t1, t0, units = "secs"))
    ), PROGRESS_BATCHES)
    ...
    cat(glue("[{format(Sys.time(), '%H:%M:%S')}] eff={eff_pct} batch={b_lo}-{b_hi} ok_cum={ok_c_n}/{n_done} fail_rate={round(fail_rate_cum,3)} power_so_far={ifelse(is.finite(power_so_far), round(power_so_far,3), NA)}\n"))
    flush.console()
...
if (any(abs(pwr$effect_pct) < 1e-12)) {
  ...
  diag_out <- if (use_parallel) {
    parallel::parLapply(cl, sim_seeds[seq_len(n_diag)], one_sim, effect_log = 0.0)
  } else {
    lapply(sim_seeds[seq_len(n_diag)], function(ss) one_sim(ss, effect_log = 0.0))
  }
  ...
  p0_ok <- p0[ok0]; p0_ok <- p0_ok[is.finite(p0_ok)]
  if (length(p0_ok) >= 2L) {
