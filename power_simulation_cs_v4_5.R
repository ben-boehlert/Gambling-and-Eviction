...
safe_future_map_draws <- function(n_sims, one_draw_fn, eff, cfg) {
  ...
  for (w in worker_try) {
    ...
    out <- tryCatch({
      # Do NOT use progressr from within parallel workers; it can emit noisy "not listening" warnings
      # when futures error/retry. Keep logging at the effect/scenario level instead.
